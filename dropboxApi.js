...
async function authorizeDropbox(req, res) {
    try {
        ...
        const authUrl = await dropbox.getAuthenticationUrl(REDIRECT_URI, null, 'code', 'offline', null, 'none', false);
...
async function dropboxCallback(req, res) {
    try {
        ...
        const { access_token: accessToken, refresh_token: refreshToken } = response.result;
